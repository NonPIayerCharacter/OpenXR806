 ...
 *
 * Param txpower support range is -26dBm ~ 18dBm [unit: dBm], acture
 * tx power will only change in a step of 3dBm, such as 0dBm/3dBm/6dBm.
 * Setting power is only effect before ble adv/scan/conn/..etc enable.
 *
 * @param txpower    transmit power supported by the PHY [unit: dBm].
 *
 * @return 0 on success or negative error value on failure.
 */
int bt_le_txpower_set(int txpower);
