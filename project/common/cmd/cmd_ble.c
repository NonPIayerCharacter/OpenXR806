...
static int cmd_set_txpower(const struct shell *shell, size_t argc, char *argv[])
{
	int val;
	int err;
	int cnt;

	cnt = cmd_sscanf(argv[1], "%d", &val);
	if (cnt != 1) {
		CMD_ERR("invalid param %d", val);
		return ENOEXEC;
	}

	err = bt_le_txpower_set(val);
	if (err < 0) {
		shell_error(shell, "Failed to start set txpower (err %d)", err);
		return err;
	}
...
#endif
	SHELL_CMD_ARG(set_power, NULL, NULL, cmd_set_txpower, 2, 0),
